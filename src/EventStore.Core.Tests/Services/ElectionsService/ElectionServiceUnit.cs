...
namespace EventStore.Core.Tests.Services.ElectionsService
{
    public class ElectionsServiceUnit
    {
        ...
        public IPEndPoint OwnEndPoint { get { return InitialClusterSettings.Self.NodeInfo.InternalHttp; } }
        ...
        {
            var members =
                (new[] { MemberInfo.ForManager(Guid.Empty, InitialDate, true, clusterSettings.ClusterManager, clusterSettings.ClusterManager) })
                .Union(new[] 
                       { 
        ...
        public IEnumerable<MemberInfo> ListMembers(Func<MemberInfo, bool> predicate = null)
        {
            ...
            return ClusterInfo.Members.Where(predicate).Select(x => 
                    x.State == VNodeState.Manager
                        ? MemberInfo.ForManager(x.InstanceId, x.TimeStamp, x.IsAlive, x.InternalHttpEndPoint, x.ExternalHttpEndPoint)
